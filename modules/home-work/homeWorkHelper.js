...
homeWorkUtil.savehomeWorkImage = async(file, old_path, cb) => {
    if (!utils.empty(file) && !utils.empty(file.document) && file.document.length > 0) {
        utils.imageArr = [];
console.log("homework file".file)
//  await cloudinary.uploader.upload(file, {
//             resource_type: 'auto',
//             folder: 'eventPoster',
//           })
        utils.uploadImageInServermulti(file.document, config.HOMEWORK_IMAGE_PATH, 0, cb);
    } else {
        cb(null, null);
    }
};
